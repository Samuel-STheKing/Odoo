...
class TestHelpdeskTicketProject(TestHelpdeskTicketBase):
    ...
    def setUpClass(cls):
        ...
        cls.ticket.write({
            ...
            "task_ids": [(6, 0, [cls.task_project1.id])]
        })
        cls.ticket2.write({
            "project_id": cls.project1.id,
            "task_ids": [(6, 0, [cls.task_project1.id])]
    ...
    def test_helpdesk_ticket_project_task(self):
        """Al cambiar de proyecto se deben limpiar las tareas"""
        self.ticket.write({"project_id": self.project2.id})
        self.assertFalse(
            self.ticket.task_ids,
            "Helpdesk Ticket: When changing the project the ticket tasks should be reset.",
        )

    def test_helpdesk_ticket_counts(self):
        self.assertEqual(
            self.project1.ticket_count, 2,
            "Helpdesk Ticket: Project should have two related tickets."
        )
        self.assertEqual(
            self.project1.todo_ticket_count, 2,
            "Helpdesk Ticket: Project should have two related todo tickets."
        )
        self.assertEqual(
            self.task_project1.ticket_count, 2,
            "Helpdesk Ticket: Task should have two related tickets."
        )
        self.assertEqual(
            self.task_project1.todo_ticket_count, 2,
            "Helpdesk Ticket: Task should have two related todo tickets."
        )
        self.assertEqual(self.project2.ticket_count, 0)
        self.assertEqual(self.task_project2.ticket_count, 0)

        # Cerrar un ticket
        self.ticket.write({"stage_id": self.stage_closed.id})
        self.assertEqual(self.project1.todo_ticket_count, 1)
    ...
    def test_action_open_helpdesk_tickets_from_task(self):
        """Test action from task"""
        action = self.task_project1.action_open_helpdesk_tickets_from_task()
        self.assertIn("task_ids", action.get("domain", []))
    ...
    def test_project_update_buttons(self):
        ...
        self.assertFalse(
            any(button["action"] == "action_open_helpdesk_tickets" for button in buttons)
        )
    ...
    def test_milestones(self):
        ...
        # Asignar tarea (Many2many)
        ticket_1.write({"task_ids": [(6, 0, [task.id])]})
        ...
        milestone_requests = self.env[action["res_model"]].search(
            safe_eval(action["domain"], {"active_id": self.milestone.id})
        )
